...
void main() {
  ...
  test('落盘库初始化后文件真实存在且八张表建成', () async {
  ...
  test('数据在重开连接后仍然存在', () async {
    ...
    // 重开后 user_version 仍是 1，不会重跑 onCreate。
    final v = await db.customSelect('PRAGMA user_version').getSingle();
    expect(v.data.values.first, 1);
  });
  ...
  test('落盘库在 500 客户 + 5000 跟进下排序仍低于 200ms', () async {
    ...
    expect(samples[1], lessThan(const Duration(milliseconds: 200).inMicroseconds));
  });
}
